...
namespace Task21
{
    internal class MyTreeMap<K, T>
    {
        ...
        public void AddTree(Entry<K, T> x, Tree<K, T> root1)
        {
            ...
            if (Comparer<K>.Default.Compare(x.key, root1.info.key) == 1)
            {
                if (root1.r == null)
                {
                    root1.r = new Tree<K, T>(x);
        ...
        public void Remuve(K key)
        {
            Remuve(key, root, null, false);
        }
        public void Remuve(K Key, Tree<K, T> root1, Tree<K, T> rootMuter, bool left)
        {
            if (root1 == null)
            {
                return;
            ...
            {
                Remuve(Key, root1.r, root1, false);
            }
            else
            {
                Remuve(Key, root1.l, root1, true);
            }
        }
